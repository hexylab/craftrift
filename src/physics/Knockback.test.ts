...
describe('KnockbackState', () => {
  it('only has vx and vz fields', () => {
    const state = createKnockbackState();
    expect(Object.keys(state)).toEqual(['vx', 'vz']);
    expect('vy' in state).toBe(false);
  });
});

describe('applyKnockback', () => {
  it('applies velocity away from source', () => {
    const state: KnockbackState = { vx: 0, vz: 0 };
  ...
  it('handles zero distance', () => {
    ...
    expect(state.vz).toBeCloseTo(KNOCKBACK_HORIZONTAL);
  });
});

describe('updateKnockback', () => {
  it('decays velocity over time', () => {
    const state: KnockbackState = { vx: 3.0, vz: 0 };
  ...
  it('reaches near-zero after sufficient time', () => {
    const state: KnockbackState = { vx: 3.0, vz: 3.0 };
...
describe('KNOCKBACK_VERTICAL', () => {
  it('is exported for use by EntityPhysics', () => {
    expect(KNOCKBACK_VERTICAL).toBe(2.0);
  });
});
