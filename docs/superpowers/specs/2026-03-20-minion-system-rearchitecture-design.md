...
- `moveWithCollision`: Player.moveAxis相当のAABBステッピング衝突
...
- `Pathfinding.ts:163` のフォールバック修正: A*失敗時に空配列を返す（その場で待機、次回再計算時にリトライ）
...
| Knockback.ts:37 | 未使用`vy`フィールド削除 |
...
- プレイヤー⇔ミニオン衝突: 押し出し比率、境界条件
- パスファインディング: フォールバック時の挙動、マージン対称性
- 全テストスイートがパスすること
